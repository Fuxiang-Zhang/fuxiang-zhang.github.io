...
- **Progressive output.** Live chat text appears as network deltas arrive. Command output and mock replies use a word-by-word writer (`src/stream.ts`) at a fixed 40 words per second, paced by elapsed time so the speed is independent of frame rate and text length. The page follows the newest text while writing; scrolling up stops following. The opening screen appears at once. While output is printing, the status bar reads `printing output`, and questions, commands and paper cards wait until it has finished, so only one piece of output prints at a time; a hash navigation completes the running output immediately instead. Appending output preserves existing DOM nodes and animations.
...
`chat/openai.ts` uses the streaming OpenAI Responses API with low verbosity, low reasoning effort, a 4096-token output limit per response, and `store: false`. `OPENAI_MODEL` defaults to `gpt-5.6-terra`. Stable developer instructions and reference material precede a separate dynamic developer message and conversation history. Recognized GPT-5.6+ model names use `prompt_cache_options: { mode: "explicit", ttl: "30m" }` and a breakpoint on the stable block. Unknown/custom or earlier model names omit these model-specific settings and use provider defaults. The cache key identifies the stable content and model; it does not guarantee a hit or retention.

`CHAT_CONTEXT_MODE=selective` is the default in the local server and Worker. `chat/context.ts` projects profile, experience, research and a complete publication index from `SiteData`, preserving authored contributions and roles while excluding full abstracts and presentation fields. `chat/papers.ts` preloads explicitly named papers by ID, full title or curated alias; otherwise the selected paper provides context for ambiguous questions. The model can batch missing details through `get_papers({ids})`. Details are homepage metadata and abstracts, not fetched full papers. At most two lookup rounds and three model responses share the original 60-second deadline. The last response disables tools; continuation preserves function call IDs and encrypted reasoning. Tool definitions stay unchanged across rounds.

Set `CHAT_CONTEXT_MODE=full` to restore full-source context without paper tools. This rollback still uses the improved cache layout and accounting. Date and selected focus remain dynamic; an explicit different topic takes precedence. Both modes retain first-person, source-grounded answers, language matching and the off-topic protocol. Replies default to one or two paragraphs and can expand for detailed comparisons or complete lists. The model continues to emit `[[paper:id]]`; frontend cards, deduplication and the three-card display limit are unchanged. That display limit does not restrict paper retrieval.
...
| `OPENAI_MODEL` | `gpt-5.6-terra` | Model to use |

Hourly counts and the `usage:YYYY-MM-DD` ledger are updated atomically in SQLite-backed Durable Objects, with one object per counter key. Daily records expire after 90 days. The local Node server uses an atomic in-memory store and the same handler. `requests` counts successful replies; `errors` counts failed user requests. `modelCalls` counts attempted provider calls across all tool rounds; `cacheWrite` records provider cache-write tokens. `incompleteUsage` counts attempts for which final provider usage was unavailable, so unknown charges are not represented as measured zero. Old ledger records default the new fields to zero. `reasoning` is the share of `output` the model spent thinking before the visible answer; `cached` is the share of `input` served from the provider's prompt cache. When an incomplete, refused or empty reply includes provider usage, those tokens are also recorded. If a later tool round fails, known usage from earlier rounds is still recorded once. Unknown usage cannot be reconstructed locally. Structured `chat: model usage` logs include model, context mode, response ID, cumulative usage, first text latency and total duration, without question or answer text. Token counters are not a dollar-cost estimate: apply the model’s current input, cache-read, cache-write and output pricing to these measurements; reasoning is already included in output. Storage failures return 503 instead of silently resetting the budget.
...
On every push to `main` or `reading-mode`, `.github/workflows/static.yml` checks out both branches. It installs the terminal homepage dependencies from `main`, type-checks and runs its tests (which build `dist/`), then copies that output to the site root. The static homepage from `reading-mode` is copied to `/reading/`. The combined site is uploaded and deployed once. Public URLs are `https://fuxiang-zhang.github.io/` and `https://fuxiang-zhang.github.io/reading/`. The legacy `/vibe/` URL redirects to the root, preserving query parameters and section hashes when JavaScript is enabled.

Keep this workflow identical on both branches. In repository Settings → Pages, use GitHub Actions as the source; the `github-pages` environment must allow deployments from `main` and `reading-mode`. Both branches must exist remotely. A failed terminal build prevents the combined deployment and leaves the previous live site intact. Reading-mode resources must remain relative so they resolve under `/reading/`. The terminal preview uses the production chat Worker; use `/?chat=mock` for mock replies. The build publishes `.build/` and `dist/` only after success; generated JavaScript is not committed.
...
| `chat/` | `context.ts` (compact factual projection), `papers.ts` (paper lookup and preloading); platform-independent backend: `handler.ts` (request handling), `limits.ts` (rate limits and the usage ledger), `openai.ts` (model call), `prompt.ts` (instructions) |
...
### Chat optimization validation

Run `npm test` for offline mocked provider coverage, including tool continuation, rollback, cache configuration, multiple-call accounting and lost final usage. `tests/chat-eval.json` contains 31 bilingual quality cases for live evaluation. Compare the original full-context implementation, `full` with improved caching, and `selective`, using the same model and generation settings. Evaluate both cold and reused prefixes; identical cache keys alone do not establish a warm cache. Review bibliographic accuracy, completeness, grounded claims and citation IDs, and compare cumulative input/cache-write/output usage and latency across all calls. Do not report character reduction as measured token or monetary savings. Real API quality and cost evaluation is a separate paid operation; the offline tests never contact OpenAI.

Run the paid smoke and lookup checks explicitly with `npm run build && EVAL_CASES=0,30 node --env-file=.env scripts/eval-chat-live.mjs --live`. Without `EVAL_CASES`, the script runs 15 selected cases, including the required paper lookup. `EVAL_MODE=full` selects the full-context comparison; `EVAL_OUTPUT` overrides the default `/tmp/homepage-live-eval.json` report path. Any failed automated check produces a nonzero exit code. A passing result checks protocol and selected factual fields, not comprehensive answer quality; review the saved answers as well.
